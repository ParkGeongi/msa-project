...
class Korean_Classiy_Service(object):

    def __init__(self):
        pass

    def hook(self,**keyargs):
        ...
        x_test = [self.count_codePoint(keyargs['quiz'])]

        y_pred = clf.predict(x_test)
        y_test = [keyargs['answer']]
        print(f'예측 값 : {y_pred}')
        print(f'실제 값 : {y_test}')
        print(f'3개 중 맞춘 확률 : {accuracy_score(y_test,y_pred)}')

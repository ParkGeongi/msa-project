...
from app.services.chatbot.kakao_chatbot import Chatbot
...
class ChatbotDataset(Dataset):
    def __init__(self, chats, max_len=40):  # 데이터셋의 전처리를 해주는 부분
        ...
        self.eos = EOS
        self.mask = MASK
        self.tokenizer = PreTrainedTokenizerFast.from_pretrained("skt/kogpt2-base-v2", bos_token=BOS, eos_token=EOS,
                                                           unk_token="<unk>", pad_token=PAD, mask_token=MASK, )
...
class Kogpt2:
    ...
    def exec(self):
        ...
        torch.save(model.state_dict(), "./save/model_state_dict.pt")

    def test(self,request_user:Chatbot):


        device = "cpu"
        root = r"C:\Users\AIA\project\FastApi\app\services\chatbot\save\model.pt"
        path = "/usr/src/app/app/services/chatbot/save/model.pt"

        model = torch.load(path, map_location=device)
        msg = request_user.prompt
        with torch.no_grad():
            while 1:
                #q = input("user > ").strip()
                q = msg
